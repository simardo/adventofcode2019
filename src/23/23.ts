...
class IntCode {
    ...
    public print(prefix: number): void {
        let lines: string[] = [];
        let line: string = '';
...
for (let i: number = 0; i < 50; i++) {
    computers[i] = { ip: i, nic: new IntCode(0, [...INPUT_23]), queue: [] };
}
computers[255] = { ip: 255, nic: undefined!, queue: [] };
...
// uncomment part 2 to execute part 1
// console.log('part 1');

// while (!stop) {
//     Object.values(computers).forEach(c => {
//         if (c.ip !== 255) {
//             while (c.nic.outputs.length > 0) {
//                 const address: number = c.nic.outputs.shift()!;
//                 const packet: Packet = { x: c.nic.outputs.shift()!, y: c.nic.outputs.shift()! };
//                 if (address === 255) {
//                     stop = true;
//                     console.log(255, packet);
//                 } else {
//                     computers[address].queue.push(packet);
//                 }
//             }
//         }
//     });
...
const resumeYs: {[keys: number]: boolean} = {};
...
while (!stop) {
        ...
        if (c.ip !== 255) {
            while (c.nic.outputs.length > 0) {
                ...
                if (address === 255) {
                    console.log(255, packet);
                    computers[255].queue = [];
                    computers[255].queue.push(packet);
                } else {
                    computers[address].queue.push(packet);
                }
            }
    ...
    if (idle) {
        if (++idleCount === 2) {
            const resume: Packet = computers[255].queue.shift()!;
            console.log('idle', resume);
            if (resumeYs[resume.y] !== undefined) {
                console.log(resume.y);
                stop = true;
            }
            resumeYs[resume.y] = true;
            computers[0].queue.push(resume);
            idleCount = 0;
        }
    }
}
