...
function getKeyring(tile: SuperTunnelTileMap, keys: number): number {
    ...
    if (isKey) {
        ...
        const k: string = tile.t;
        const nk: number = keys | Math.pow(2, k.charCodeAt(0) - 'a'.charCodeAt(0));
        if (nk !== keys && kkk[nk] === undefined) {
            kkk[nk] = 1;
            console.log(keys, '-->', nk, k, Object.keys(scoredNodes).length);
        }
        return nk;
    } else {
        return keys;
...
function doPart2(input: string): void {
    ...
    console.log(mapSet['1']);
    console.log(mapSet['2']);
    console.log(mapSet['3']);
    console.log(mapSet['4']);
    console.log(mapSet['a']);
    console.log(mapSet['c']);
    console.log(mapSet['b']);
    console.log(mapSet['d']);
