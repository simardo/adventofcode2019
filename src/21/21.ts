...
const program1: number[] = instructions1.reduce((acc: number[], i) => acc.concat(convert(i)), []);

const springscript1: IntCode = new IntCode(0, [...INPUT_21]);
program1.forEach((p, i) => springscript1.run(p));

let lines: string[] = [];
let line: string = '';

springscript1.outputs.forEach((o, i) => {
    if (o === 10) {
        lines.push(line);
        line = '';
    } else {
...
console.log(springscript1.outputs);
...
const program2: number[] = instructions2.reduce((acc: number[], i) => acc.concat(convert(i)), []);

const springscript2: IntCode = new IntCode(0, [...INPUT_21]);
program2.forEach((p, i) => springscript2.run(p));

lines = [];
line = '';

springscript2.outputs.forEach((o, i) => {
    if (o === 10) {
        lines.push(line);
        line = '';
    } else {
...
console.log(springscript2.outputs);
